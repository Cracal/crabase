...
test_int_float(void)
{
    ...
    assert_always(cra_cmp_double((double)in_i.n, out_f.n) == 0);
    assert_always(cra_cmp_double((double)in_i.W, out_f.W) == 0);
#ifdef CRA_JSON_SAFE_INT
    assert_always(cra_cmp_double((double)CRA_MIN_SAFE_INT, out_f.m) == 0);
    assert_always(cra_cmp_double((double)CRA_MAX_SAFE_INT, out_f.M) == 0);
    assert_always(cra_cmp_double((double)CRA_MAX_SAFE_INT, out_f.UM) == 0);
#else
...
CRA_TYPE_META_MEMBER_LIST(struct N, l, 11, true, metal, NULL, NULL, NULL, NULL)
CRA_TYPE_META_MEMBER_DICT(struct N, c, 12, false, metac, NULL, NULL, NULL, NULL)
// old
...
test_struct_xx(void)
{
    ...
    CRA_TYPE_META_SET_I(&metan[10], CRA_LLIST_ITERABLE_I, CRA_LLIST_APPENDABLE_I, CRA_LLIST_INITIALIZABLE_I, &llargs);
    CRA_DICT_INITIALIZABLE_PARAM_DEF(dargs, int32_t, int32_t, cra_hash_int32_p, cra_cmp_int32_p);
    CRA_TYPE_META_SET_I(&metan[11], CRA_DICT_ITERABLE_I, CRA_DICT_APPENDABLE_I, CRA_DICT_INITIALIZABLE_I, &dargs);
