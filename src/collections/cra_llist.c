...
CRA_FOREACH_NEXT_DEF(CraLList)
{
    ...
    if (retval)
        memcpy(retval, it->current->val, obj->itemsize);
    it->current = it->current->next;
    if (it->current == obj->head)
        it->current = NULL;
    return true;
}

CRA_FOREACH_PREV_DEF(CraLList)
{
    assert(it);
    assert(obj);
    ...
    {
        it->initialized = true;
        it->current = obj->head ? obj->head->prev : NULL;
    }
